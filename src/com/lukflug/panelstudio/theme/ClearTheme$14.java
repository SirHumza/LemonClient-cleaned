...
implements ISwitchRenderer<String> {
    ...
    ClearTheme.14(boolean bl, int n, int n2) {
    ...
    public void renderButton(Context context, String title, boolean focus, String state) {
        ...
        ClearTheme.this.getSmallButtonRenderer(5, this.val$logicalLevel, this.val$graphicalLevel, this.val$container).renderButton(subContext, null, effFocus, null);
        rect = this.getOffField(context);
        subContext = new Context(context, rect.width, new Point(rect.x - context.getPos().x, 0), true, true);
        subContext.setHeight(rect.height);
        ClearTheme.this.getSmallButtonRenderer(4, this.val$logicalLevel, this.val$graphicalLevel, false).renderButton(subContext, null, effFocus, null);
    }
