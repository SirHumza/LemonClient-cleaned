...
implements IComponentAdder {
    ...
    public <S extends IComponent, T extends IComponent> void addComponent(S title, T content, ThemeTuple theme, Point position, int width, Supplier<Animation> animation) {
        AnimatedToggleable toggle = new AnimatedToggleable(new SimpleToggleable(this.open), animation.get());
        RendererTuple<Void> renderer = new RendererTuple<Void>(Void.class, theme);
        IResizable size = this.getResizable(width);
        this.container.addComponent(ResizableComponent.createResizableComponent(title, content, () -> null, toggle, renderer, theme.theme.getResizeRenderer(), size, this.getScrollSize(size), position, width, true, (String)this.configName.apply(content.getTitle())), this.isVisible);
    }
