...
implements IComponentAdder {
    ...
    public SinglePanelAdder(IContainer<? super IFixedComponent> container, ILabeled label, ITheme theme, Point position, int width, Supplier<Animation> animation, IBoolean isVisible, String configName) {
        ...
        container.addComponent(ResizableComponent.createResizableComponent(this.title, this.content, () -> null, toggle, renderer, theme.getResizeRenderer(), size, new IScrollSize(){
    ...
    public <S extends IComponent, T extends IComponent> void addComponent(S title, T content, ThemeTuple theme, Point position, int width, Supplier<Animation> animation) {
        ...
        this.content.addComponent(new HorizontalComponent<2>(new ScrollBarComponent<Void, T>(content, theme.getScrollBarRenderer(Void.class), theme.getEmptySpaceRenderer(Void.class, false), theme.getEmptySpaceRenderer(Void.class, true)){
