...
extends Module {
    ...
    BooleanSetting letter = this.registerBoolean("Letter", true, () -> (Boolean)this.randomThing.getValue());
    BooleanSetting number = this.registerBoolean("Number", true, () -> (Boolean)this.randomThing.getValue());
    IntegerSetting character = this.registerInteger("Character", 20, 0, 256, () -> (Boolean)this.randomThing.getValue());
    ...
    private final Listener<PacketEvent.Receive> receiveListener = new Listener<PacketEvent.Receive>(event -> {
        if (AutoSpam.mc.field_71441_e == null || AutoSpam.mc.field_71439_g == null || !((Boolean)this.hide.getValue()).booleanValue()) {
            return;
        }
        if (event.getPacket() instanceof SPacketChat) {
            ...
            if (message.toLowerCase().contains("to") && message.contains(":") || username.equals(AutoSpam.mc.field_71439_g.func_70005_c_())) {
                event.cancel();
    ...
    @Override
    public void onTick() {
        if (AutoSpam.mc.field_71441_e == null || AutoSpam.mc.field_71439_g == null) {
            return;
        }
        ...
        if (((Boolean)this.randomThing.getValue()).booleanValue()) {
            String characters = ((Boolean)this.letter.getValue() != false ? "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" : "") + ((Boolean)this.number.getValue() != false ? "0123456789" : "");
