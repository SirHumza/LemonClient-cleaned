...
extends Module {
    ...
    IntegerSetting duplicatesTimeout = this.registerInteger("Duplicates Timeout", 30, 1, 600, () -> (Boolean)this.duplicates.getValue());
    ...
    IntegerSetting ignoreDuration = this.registerInteger("Ignore Duration", 120, 0, 43200, () -> (Boolean)this.autoIgnore.getValue());
    IntegerSetting violations = this.registerInteger("Violations", 3, 1, 100, () -> (Boolean)this.autoIgnore.getValue());
    private final Pattern CHAT_PATTERN = Pattern.compile("<.*?> ");
    private ConcurrentHashMap<String, Long> messageHistory;
    public List<String> ignoredBySpamCheck = new ArrayList<String>();
    public Map<String, Integer> violate = new ConcurrentHashMap<String, Integer>();
    public List<String> ignoredList = new ArrayList<String>();
    @EventHandler
    private final Listener<PacketEvent.Receive> receiveListener = new Listener<PacketEvent.Receive>(event -> {
        int spaceIndex;
        if (AntiSpam.mc.field_71439_g == null || !this.isEnabled()) {
            return;
        }
        ...
        } else if (s.contains(":") && (spaceIndex = s.indexOf(" ")) != -1) {
            username = s.substring(0, spaceIndex);
        }
        username = AntiSpam.cleanColor(username);
        if (username.equals("null") || AntiSpam.mc.field_71439_g.field_71174_a.func_175104_a(username) == null || SocialManager.isIgnore(username) || ((Boolean)this.filterFriend.getValue()).booleanValue() && SocialManager.isOnFriendList(username)) {
            return;
        }
        SPacketChat sPacketChat = (SPacketChat)event.getPacket();
        if (this.detectSpam(sPacketChat.func_148915_c().func_150260_c()) && !username.equalsIgnoreCase(AntiSpam.mc.field_71439_g.func_70005_c_())) {
            if (((Boolean)this.autoIgnore.getValue()).booleanValue()) {
                if (this.violate.get(username) != null && this.violate.get(username) >= (Integer)this.violations.getValue() || (Integer)this.violations.getValue() == 0) {
    ...
    public void startIgnore(final String finalUsername) {
        if (AntiSpam.mc.field_71439_g.func_70005_c_().equalsIgnoreCase(finalUsername)) {
            return;
        }
        ...
        }, (Integer)this.ignoreDuration.getValue() * 1000);
    }
    ...
    private boolean detectSpam(String message) {
        if (((Boolean)this.greenText.getValue()).booleanValue() && this.findPatterns(FilterPatterns.GREEN_TEXT, message)) {
        ...
        if (((Boolean)this.discordLinks.getValue()).booleanValue() && this.findPatterns(FilterPatterns.DISCORD, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Discord Link: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.webLinks.getValue()).booleanValue() && this.findPatterns(FilterPatterns.WEB_LINK, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Web Link: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.ips.getValue()).booleanValue() && this.findPatterns(FilterPatterns.IP_ADDR, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] IP Address: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.ipsAgr.getValue()).booleanValue() && this.findPatterns(FilterPatterns.IP_ADDR_AGR, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] IP Aggressive: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.tradeChat.getValue()).booleanValue() && this.findPatterns(FilterPatterns.TRADE_CHAT, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Trade Chat: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.numberSuffix.getValue()).booleanValue() && this.findPatterns(FilterPatterns.NUMBER_SUFFIX, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Number Suffix: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.announcers.getValue()).booleanValue() && this.findPatterns(FilterPatterns.ANNOUNCER, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Announcer: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.spammers.getValue()).booleanValue() && this.findPatterns(FilterPatterns.SPAMMER, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Spammers: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.insulter.getValue()).booleanValue() && this.findPatterns(FilterPatterns.INSULTER, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Insulter: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.greeters.getValue()).booleanValue() && this.findPatterns(FilterPatterns.GREETER, message)) {
            if (((Boolean)this.showBlocked.getValue()).booleanValue()) {
                MessageBus.sendMessage("[AntiSpam] Greeter: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
            }
            return true;
        }
        if (((Boolean)this.duplicates.getValue()).booleanValue()) {
            if (this.messageHistory == null) {
                this.messageHistory = new ConcurrentHashMap();
            }
            boolean isDuplicate = this.messageHistory.containsKey(message) && (System.currentTimeMillis() - this.messageHistory.get(message)) / 1000L < (long)((Integer)this.duplicatesTimeout.getValue()).intValue();
