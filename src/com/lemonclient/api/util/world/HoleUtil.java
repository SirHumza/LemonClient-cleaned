...
public class HoleUtil {
    ...
    public static boolean isInHole(Entity e, boolean onlyOneWide, boolean ignoreDown, boolean render) {
        return !HoleUtil.isHole(new BlockPos(e.func_174791_d()), onlyOneWide, ignoreDown, render).getType().equals((Object)HoleType.NONE);
    }

    public static boolean isHoleBlock(BlockPos blockPos, boolean onlyOneWide, boolean ignoreDown, boolean render) {
        return !HoleUtil.isHole(blockPos, onlyOneWide, ignoreDown, render).getType().equals((Object)HoleType.NONE) && !HoleUtil.isHole(blockPos, onlyOneWide, ignoreDown, render).getType().equals((Object)HoleType.CUSTOM);
    }
    ...
    public static HoleInfo isHole(BlockPos centreBlock, boolean onlyOneWide, boolean ignoreDown, boolean render) {
        ...
        if (size == 2 && !onlyOneWide) {
            return HoleUtil.isFourHole(output, centreBlock, render);
        }
        output.setSafety(BlockSafety.BREAKABLE);
        return output;
    }
    ...
    private static HoleInfo isFourHole(HoleInfo info, BlockPos centreBlock, boolean render) {
        if (render ? HoleFinder.is2x2single(centreBlock, true) : HoleFinder.is2x2(centreBlock, true)) {
            ...
            double maxX = centreBlock.func_177958_n() + 2;
            double minZ = centreBlock.func_177952_p();
            double maxZ = centreBlock.func_177952_p() + 2;
            if (!render) {
                BlockPos blockPos = centreBlock;
                if (HoleUtil.mc.field_71441_e.func_175623_d(centreBlock.func_177982_a(-1, 0, 0))) {
                    blockPos = blockPos.func_177982_a(-1, 0, 0);
                }
                if (HoleUtil.mc.field_71441_e.func_175623_d(centreBlock.func_177982_a(0, 0, -1))) {
                    blockPos = blockPos.func_177982_a(0, 0, -1);
                }
                minX = blockPos.func_177958_n();
                maxX = blockPos.func_177958_n() + 2;
                minZ = blockPos.func_177952_p();
                maxZ = blockPos.func_177952_p() + 2;
            }
            info.setCentre(new AxisAlignedBB(minX, (double)centreBlock.func_177956_o(), minZ, maxX, (double)(centreBlock.func_177956_o() + 1), maxZ));
