...
public static class Easing.ElasticIn
extends Easing.Elastic {
    public Easing.ElasticIn(float amplitude, float period) {
    ...
    public float ease(float time, float startTime, float change, float endTime) {
        ...
        return -(a * (float)Math.pow(2.0, 10.0f * (time -= 1.0f)) * (float)Math.sin((double)(time * endTime - s) * (Math.PI * 2) / (double)p)) + startTime;
    }
}
