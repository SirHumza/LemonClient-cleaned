...
extends Notification {
    public Map<Integer, Integer> offsetDirLeft = new HashMap<Integer, Integer>();
    public Map<Integer, Integer> offsetDirRight = new HashMap<Integer, Integer>();
    ...
    public void render(int RealDisplayWidth, int RealDisplayHeight) {
        ...
        if (shouldEffect) {
            ...
            for (Map.Entry<Integer, Integer> entry : this.offsetDirLeft.entrySet()) {
                int n = entry.getValue();
                alpha = TopNotification.calculateAlphaChangeColor(255, 10, 50, n);
                RenderUtil.setColor(new Color(color.getRed(), color.getGreen(), color.getBlue(), alpha));
                this.drawC(x - n, cy, height, height);
            }
            for (Map.Entry<Integer, Integer> entry : this.offsetDirRight.entrySet()) {
                int n = entry.getValue();
                alpha = TopNotification.calculateAlphaChangeColor(255, 10, 50, n);
                RenderUtil.setColor(new Color(color.getRed(), color.getGreen(), color.getBlue(), alpha));
                this.drawBC(x + dWidth + n, cy, height, height);
            }
        }
        int delay = 3;
        if (this.delaydir >= 3) {
        ...
        RenderUtil.drawTriangle(x, cy, x - 15 - 1, (double)cy + (double)height / 2.0, x, cy + height, color);
        RenderUtil.drawTriangle(x + dWidth, cy + height, x + 15 + dWidth, (double)cy + (double)height / 2.0, x + dWidth, cy, color);
        int n = x + dWidth / 2;
        int n2 = TopNotification.calculateAlphaChangeColor(10, 255, width, offset);
        font.drawString(this.title, (float)n - (float)font.getStringWidth(this.title) / 2.0f, cy + 3, new GSColor(255, 255, 255, n2));
        font.drawString(this.message, (float)n - (float)font.getStringWidth(this.message) / 2.0f, cy + font.getHeight() + 8, new GSColor(255, 255, 255, n2));
    ...
    public void drawBC(int cx, int cy, int height, int margin) {
        ...
        GL11.glVertex2d((double)(cx + margin), (double)((double)cy + (double)height / 2.0));
